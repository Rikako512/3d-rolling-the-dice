...
{   
    ...
    public GameObject matrixScatterplot; // クリックされたオブジェクト ≒ 生成されるオブジェクトの元となるオブジェクト
    // DataCube のマトリックスの場合、クリックされるのは DataCube であり、元となるオブジェクトも DataCub
    // Scatterplot のマトリックスの場合、クリックされるのは Scatterplot の Cube であり、元となるオブジェクトは Scatterplot
    ...
    // Scatterplot の Cube または DataCube がクリックされたら実行
    public void OnPointerClick(PointerEventData eventData)
    {
      ...
      {
        // クリックしたオブジェクトが Scatterplot の場合
        if(matrixScatterplot.transform.tag == "Scatterplot")
        { 
           // クリックしたオブジェクトの3つの次元に対応する値を取得する
           PointRenderer scatterplotter = matrixScatterplot.transform.Find("GraphFrame/Plotter").gameObject.GetComponent<PointRenderer>();
           click_X = scatterplotter.column1;
           click_Y = scatterplotter.column2;
           click_Z = scatterplotter.column3;
    ...
    private void PlacePrefabLargeGraph()
    {
      ...
      // 大きく表示するオブジェクトを生成
      GameObject point_plotter = matrixScatterplot.transform.Find("GraphFrame/Plotter").gameObject;
      plotterScript = point_plotter.GetComponent<PointRenderer>();

      /*
      if (plotterScript.renderPointPrefabs == true) // 散布図行列上で全てのデータ点を描画している場合
      {
          point_plotter.SetActive(false); // データ点が2重に作成されてしまうのを防ぐため、無効にしておく
      }else{
          // クラスタリング後のデータ点を散布図行列上で表示している場合
          plotterScript.renderPointPrefabs = true;
      }
      */

      if (plotterScript.renderPointPrefabs == false) // クラスタリング後のデータ点を散布図行列上で表示している場合
      {
         // クラスタリング後のデータ点を散布図行列上で表示している場合
          plotterScript.renderPointPrefabs = true;
      }

      GameObject graph = Instantiate (matrixScatterplot, new Vector3(0, 3, -50),  Quaternion.Euler(0, 180f, 0));
      graph.transform.localScale = new Vector3(2.0f, 2.0f, 2.0f);
      ...
      // Cube は不要なので削除
      Destroy(graph.transform.Find("Cube").gameObject);

      // 生成したオブジェクトの名前を Large Graph に変更
      graph.transform.name = "Large Graph";
    ...
    {
      // Update axis labels
      GameObject.FindWithTag("label1_x").GetComponent<TextMeshProUGUI>().text = panelX.GetComponent<TextMesh> ().text;
      GameObject.FindWithTag("label1_y").GetComponent<TextMeshProUGUI>().text = panelY.GetComponent<TextMesh> ().text;
      GameObject.FindWithTag("label1_z").GetComponent<TextMeshProUGUI>().text = panelZ.GetComponent<TextMesh> ().text;
      //GameObject.FindWithTag("label2_x").GetComponent<TextMeshProUGUI>().text = panelX.GetComponent<TextMesh> ().text;
      //GameObject.FindWithTag("label2_y").GetComponent<TextMeshProUGUI>().text = panelY.GetComponent<TextMesh> ().text;
      //GameObject.FindWithTag("label2_z").GetComponent<TextMeshProUGUI>().text = panelZ.GetComponent<TextMesh> ().text;
    }
    ...
    { 
      ...
      if(previousPath != null){ // linrenderer.GameObject != null でいい？
